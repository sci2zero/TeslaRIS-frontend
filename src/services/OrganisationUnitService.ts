...
export class OrganisationUnitService extends BaseService {
  ...
  async searchOUs(tokens: string, forPersonId: number | null, topLevelInstitutionId: number | null, onlyHarvestableInstitutions: boolean | null = null, onlyIndependentInstitutions: boolean | null = null, allowedThesisType: ThesisType | null = null, onlyClientInstitutionsCris: boolean | null = null, onlyClientInstitutionsDl: boolean | null = null, registryBookRelevant: boolean | null = null): Promise<AxiosResponse<Page<OrganisationUnitIndex>>> {
    return super.sendRequest(axios.get, `organisation-unit/simple-search?${tokens}${forPersonId ? ("&personId=" + forPersonId) : ""}${topLevelInstitutionId ? ("&topLevelInstitutionId=" + topLevelInstitutionId) : ""}${onlyHarvestableInstitutions ? ("&onlyReturnOnesWhichCanHarvest=" + onlyHarvestableInstitutions) : ""}${onlyIndependentInstitutions ? ("&onlyIndependent=" + onlyIndependentInstitutions) : ""}${allowedThesisType ? ("&allowedThesisType=" + allowedThesisType) : ""}${onlyClientInstitutionsCris ? ("&onlyCrisClients=" + onlyClientInstitutionsCris) : ""}${onlyClientInstitutionsDl ? ("&onlyDlClients=" + onlyClientInstitutionsDl) : ""}${registryBookRelevant ? ("&registryBookRelevant=" + registryBookRelevant) : ""}`);
  }
