<template>
    <div justify="start">
        <v-dialog v-model="dialog" persistent>
            <v-card>
                <v-card-title>
                    <span class="text-h5">{{ $t("collaborationPublicationsLabel") + ` - ${collaborationName} - (${getCollaborationTypeTitleFromValueAutoLocale(props.collaborationType)})` }}</span>
                </v-card-title>
                <v-card-text>
                    <v-container>
                        <publication-table-component
                            ...
                            :endpoint-token-parameters="[String(props.sourcePersonId), String(props.targetPersonId), props.collaborationType]"
