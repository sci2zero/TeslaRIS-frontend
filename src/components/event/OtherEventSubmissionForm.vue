...
import { OtherEventType, type OtherEvent } from '@/models/EventModel';
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const defaultEventType = {
                title: getOtherEventTypeTitleFromValueAutoLocale(OtherEventType.LECTURE) as string,
                value: OtherEventType.LECTURE
            };
        const selectedOtherEventType = ref<{title: string, value: OtherEventType}>(defaultEventType);
        ...
        const submit = (stayOnPage: boolean) => {
            ...
            const newOtherEvent: OtherEvent = {
                name: name.value,
            ...
            EventService.createOtherEvent(newOtherEvent).then((response) => {
                if (props.inModal) {
                ...
                if (stayOnPage) {
                    ...
                    selectedOtherEventType.value = defaultEventType;
                    ...
                    router.push({ name: "otherEventLandingPage", params: {id: response.data.id} });
                }
            }).catch(() => {
                error.value = true;
                snackbar.value = true;
        ...
        return {
            ...
            name, nameAbbreviation, description, keywords, requiredSelectionRules,
            dateFrom, dateTo, eventYear, countries, selectedCountry,
            place, otherEventTypes, selectedOtherEventType, serialEvent,
