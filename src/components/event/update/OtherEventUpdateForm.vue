...
import type { OtherEvent, OtherEventType } from '@/models/EventModel';
...
export default defineComponent({
    name: "OtherEventUpdateForm",
    components: { MultilingualTextInput, DatePicker, UriInput, Toast },
    props: {
        presetEvent: {
            type: Object as PropType<OtherEvent | undefined>,
            required: true
        },
        inComparator: {
            type: Boolean,
    ...
    setup(props, { emit }) {
        ...
        const otherEventTypes = getOtherEventTypesForGivenLocale();
        const selectedOtherEventType = ref<{title: string, value: OtherEventType}>(
            {
                title: getOtherEventTypeTitleFromValueAutoLocale(props.presetEvent?.type as OtherEventType) as string,
                value: props.presetEvent?.type as OtherEventType
            }
        );
        ...
        const submit = async () => {
            ...
            const updatedEvent: OtherEvent = {
                ...
                type: selectedOtherEventType.value.value,
        ...
        return {
            isFormInputValid, isFormValid, dateRangeError, name,
            ...
            dateFrom, dateTo, countries, place, otherEventTypes, serialEvent,
            eventYear, selectedCountry, timePeriodInput, confIdValidationRules,
            sourceOpenAlexIdValidationRules, dateRangeFormatError,
            displayOrganizer, displayOrganizerRef, selectedOtherEventType,
            requiredSelectionRules, nameAbbreviation
        };
    }
});
</script>
